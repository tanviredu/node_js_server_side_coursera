...
/**
 * we redirect all the request to the secure port
 * this is done after configuring the /bin/www file 
 */
app.all('*', (req, res, next) => {
  if(req.secure){ //if the incoming request is already secure
                  // if you use https it will be set to true in www file

   return next(); // go forward               
  }else{ // if it is the insecure port
     res.redirect(307,'https://'+req.hostname+':'+app.get('secPort') + req.url);
     // we just add the https and the url with host name 
     // 307 stands for target resources is redirected and do not change the command
     // even it is redirected
  }
});
